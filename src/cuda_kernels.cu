...
__global__ void update_bc_2degree(const float *__restrict__ delta, const float *__restrict__ delta_v0, const float *__restrict__ delta_v1, const int r0, const int r1, const int r2, const LOCINT n,  float *bc, const LOCINT * __restrict__ reach, const uint64_t nvisited_v0, const uint64_t nvisited_v1,const uint64_t nvisited_v2) {
...
/// use registers for bc and delta.
	...
	if (r0 == tid)  {
		if (d_reach_v0 > 0) bc[tid] += d_reach_v0*(nvisited_v0-2);
                bc[tid] += delta[tid]*(d_reach_v2 + 1);
                bc[tid] += delta_v1[tid]*(d_reach_v1 + 1);
		return;
	}
        if (r1 == tid)  {
		if (d_reach_v1 > 0) bc[tid] += d_reach_v1*(nvisited_v1-2);
                bc[tid] += delta_v0[tid]*(d_reach_v0 + 1);
                bc[tid] += delta[tid]*(d_reach_v2 + 1);
		return;
	}
        if (r2 == tid)  {
		if (d_reach_v2 > 0) bc[tid] += d_reach_v2*(nvisited_v2-2);
                bc[tid] += delta_v0[tid]*(d_reach_v0 + 1);
                bc[tid] += delta_v1[tid]*(d_reach_v1 + 1);
		return;
	}
...
void update_bc_cuda_2degree(const uint64_t v0, uint64_t v1, uint64_t v2, int ncol, uint64_t nvisited_v0, uint64_t nvisited_v1, uint64_t nvisited_v2) {
	int r0 = -1;
        int r1 = -1;
        int r2 = -1;
        if (GI2PI(v0) == myrow) {
	...
	update_bc_2degree<<<(ncol+THREADS-1)/THREADS, THREADS>>>(d_delta, d_delta_v0, d_delta_v1, r0, r1, r2, ncol, d_bc, d_reach, nvisited_v0, nvisited_v1, nvisited_v2);
...
__global__ void scan_frt_mono_2degree_v1(
    ...
    const LOCINT *__restrict__ sigma_v1, float  *delta_v1, const int *__restrict__ lvl_v1,
    const LOCINT *__restrict__ sigma_v0, float  *delta_v0, const int *__restrict__ lvl_v0, const LOCINT __restrict__ v2dg) {
		...
		if (lvl_v1[r[k]] == depth+1) { // this is a successor
			// sigma and delta are indexed by row
			    ...
			    atomicAdd(delta_v1+c[k], a);
                        ...
                        //sigma_vo per debug
                        //aggiorno delta
                        	b = (__ldg(&delta_v0[r[k]]) + 1)* sigma_v1_ck[k] / (sigma_v1_rk[k]+sigma[r[k]]);
...
__global__ void scan_frt_mono_2degree_v2(
    ...
    const int *__restrict__ lvl, int depth,
    const LOCINT *__restrict__ sigma_v1, float  *delta_v1, const int *__restrict__ lvl_v1,
    const LOCINT *__restrict__ sigma_v0, float  *delta_v0, const int *__restrict__ lvl_v0
    , const LOCINT __restrict__ v2dg) {
		...
		if (lvl[r[k]] == depth+1) { // this is a successor
			// sigma and delta are indexed by row
			      ...
			      atomicAdd(delta+c[k], a);
                        ...
                        if (lvl_v1[r[k]] == lvl[r[k]] && v2dg != r[k]){
                        // contributo di entrambi
                        	b = (__ldg(&delta_v0[r[k]]) + 1)* sigma_ck[k] / (sigma_v1[r[k]]+sigma_rk[k]);
                        	atomicAdd(delta_v0+c[k], b);  
				continue;
                        }
                        else if (lvl_v1[r[k]] >  lvl[r[k]]){
...
LOCINT scan_frt_csc_cuda_mono_2degree(int offset, int ncol, int depth, short branch, LOCINT v2dg) {
// branch:
// 0 means v1
...
// 3 means v0... special case
	...
	// calculate degree for each vertex in frt
        if (branch == 0){
	        read_edge_count<<<(ncol+THREADS-1)/THREADS, THREADS>>>(d_deg, d_frt_start_v1+offset, ncol, d_cbuf);
        }
        else{
                read_edge_count<<<(ncol+THREADS-1)/THREADS, THREADS>>>(d_deg, d_frt_start+offset, ncol, d_cbuf);
        }
	...
	//dump_device_farray2(d_delta, row_pp, "d_delta");
        	// Store result directly into d_delta
        if (branch == 0){ //v1
	      scan_frt_mono_2degree_v1<<<blocks, THREADS>>>(d_row, d_col, nrow, d_frt_start_v1+offset, d_ncbuf, ncol,  d_sig, d_delta, d_lvl, depth, d_sig_v1, d_delta_v1, d_lvl_v1, d_sig_v0, d_delta_v0, d_lvl_v0, v2dg);
        }
        else{ //v2
              scan_frt_mono_2degree_v2<<<blocks, THREADS>>>(d_row, d_col, nrow, d_frt_start+offset, d_ncbuf, ncol, d_sig, d_delta, d_lvl, depth, d_sig_v1, d_delta_v1, d_lvl_v1, d_sig_v0, d_delta_v0, d_lvl_v0, v2dg);
        }
        MY_CUDA_CHECK( cudaEventRecord(stop, 0) );
	MY_CHECK_ERROR("scan_frt");
	MY_CUDA_CHECK( cudaEventSynchronize(stop) );
...
__global__ void init_delta_2degree(LOCINT *reach, float * delta, float * delta_v0, float * delta_v1,  int nrow) {
...
void setcuda_2degree(LOCINT reach_v0, LOCINT reach_v1, LOCINT reach_v2) {
	...
	MY_CUDA_CHECK( cudaMemset(d_sig, 0, row_pp*sizeof(*d_sig)) );
        ...
        init_delta_2degree<<<(row_pp+THREADS-1)/THREADS, THREADS>>>(d_reach, d_delta, d_delta_v0, d_delta_v1,row_pp);

	MY_CUDA_CHECK( cudaMemset(d_msk, 0, ((row_pp+BITS(d_msk)-1)/BITS(d_msk))*sizeof(*d_msk)) );

        MY_CUDA_CHECK( cudaMemcpyToSymbol(d_reach_v0, &reach_v0, sizeof(d_reach_v0), 0, cudaMemcpyHostToDevice) ); // togliere commento
        MY_CUDA_CHECK( cudaMemcpyToSymbol(d_reach_v1, &reach_v1, sizeof(d_reach_v1), 0, cudaMemcpyHostToDevice) );
        MY_CUDA_CHECK( cudaMemcpyToSymbol(d_reach_v2, &reach_v2, sizeof(d_reach_v2), 0, cudaMemcpyHostToDevice) );
        d_frt = d_frt_start;
#ifdef ONEPREFIX
	memset(tlvl, 0, sizeof(*tlvl)*MAX_LVL);
	d_cbuf = d_cbuf_start;
...
void set_mlp_cuda_2degree(LOCINT row, int level, int sigma){
          ...
          MY_CUDA_CHECK( cudaMemset(d_msk, 0, ((row_pp+BITS(d_msk)-1)/BITS(d_msk))*sizeof(*d_msk)) );
          MY_CUDA_CHECK (cudaMemcpy (d_sig_v1, d_sig, row_pp*sizeof(*d_sig_v1), cudaMemcpyDeviceToDevice ) );
          MY_CUDA_CHECK (cudaMemcpy (d_lvl_v1, d_lvl, row_pp*sizeof(*d_lvl_v1), cudaMemcpyDeviceToDevice ));
          MY_CUDA_CHECK (cudaMemcpy (d_frt_start_v1, d_frt_start, row_pp*sizeof(*d_frt_start) ,cudaMemcpyDeviceToDevice ));
          d_frt = d_frt_start;
#ifdef ONEPREFIX //blabla
	MY_CUDA_CHECK (cudaMemcpy (d_cbuf_start_v1, d_cbuf_start, MAX(row_bl,row_pp)*sizeof(*d_cbuf_start) ,cudaMemcpyDeviceToDevice ));
...
size_t initcuda(uint64_t ned, LOCINT *col, LOCINT *row) {
	...
	if (!mono) { // Run Multi-GPU code
		...
		if (heuristic == 2 || heuristic == 3 ){
				d_cbuf_start_v1 = (LOCINT *)CudaMallocSet(MAX(row_bl,row_pp)*sizeof(*d_cbuf_start_v1), 0);
	...
	d_bc    =     (float *)CudaMallocSet(row_pp*sizeof(*d_bc), 0);
		...
		d_sig_v0 = (LOCINT *)CudaMallocSet(row_pp*sizeof(*d_sig), 0);
                ...
                d_frt_start_v1 = (LOCINT *)CudaMallocSet(MAX(row_bl,row_pp)*sizeof(*d_sbuf), 0);
...
//	cub::DeviceScan::ExclusiveSum(NULL, bytes, d_cbuf, d_cbuf, row_pp);	
	...
	MY_CUDA_CHECK( cudaMemcpyToSymbol(drow_pp, &row_pp, sizeof(drow_pp),  0, cudaMemcpyHostToDevice) );
        ...
        if (heuristic == 2 || heuristic == 3 ){
              MY_CUDA_CHECK( cudaFuncSetCacheConfig(scan_frt_mono_2degree_v1, cudaFuncCachePreferL1) );
              MY_CUDA_CHECK( cudaFuncSetCacheConfig(scan_frt_mono_2degree_v2, cudaFuncCachePreferL1) );
              MY_CUDA_CHECK( cudaFuncSetCacheConfig(update_bc_2degree, cudaFuncCachePreferL1) );
