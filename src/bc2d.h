...
#define EDGE2PROC(i, j)                                                        \
    ((int)(GI2PI(i) * C + GJ2PJ(j))) // global (i,j) -> processor

// Given the vertex i, returns the processor owning the edge (i,i)
#define VERT2PROC(i) (EDGE2PROC(i, i)) // global vertex (i,)  -> processor

#define GI2LOCI(i)                                                             \
    (((i) / col_bl) * row_bl + (i) % row_bl) // global row -> local row
#define GJ2LOCJ(j) ((j) % col_bl)          // global col -> local col

#define LOCI2GI(i)                                                             \
    (((i) / row_bl) * col_bl + myrow * row_bl +                                  \
     (i) % row_bl)                          // local row -> global row
#define LOCJ2GJ(j) (mycol * col_bl + (j)) // local col -> global col

#define LIJ2LN(i, j)                                                           \
    ((i)*col_bl + (j)) // local  edge (i,j) to  local linear index
#define LIJ2GN(i, j)                                                           \
    (LOCI2GI(i) * N + LOCJ2GJ(j))    // local  edge (i,j) to global linear index
...
#define MYLOCI2LOCJ(i)                                                         \
    (((i) % row_bl) + myrow * row_bl) // local OWNED row -> local col
#define MYLOCJ2LOCI(j)                                                         \
    (((j) % row_bl) + mycol * row_bl)        // local OWNED col -> local row
...
#define CUDA_ISMYCOLL(j)                                                       \
    (((j) / drow_bl) == (LOCINT)(dmyrow)) // local col -> proc row
#define CUDA_ISMYROW(i)                                                        \
    (((i) / dcol_bl) == (LOCINT)(dmycol)) // local row -> proc row
#define CUDA_MYLOCJ2LOCI(j)                                                    \
    ((j) + (dmycol - ((j) / drow_bl)) * drow_bl) // local OWNED col -> local row
...
#define LOCJ2GJ(j)                                                             \
    (mycol * col_bl + (j)) //((j)*C + mycol)    // local col -> global col
/* UNUSED, to be modified
#define    LIJ2LN(i,j)    ((i)*col_bl + (j))        // local  edge (i,j) to
local linear index
#define    LIJ2GN(i,j)    (LOCI2GI(i)*N + LOCJ2GJ(j)) // local  edge (i,j) to
global linear index
#define GIJ2GN(i,j)    ((i)*N + (j))            // global edge (i,j) to global
linear index
...
*/
#define MYLOCI2LOCJ(i)                                                         \
    (((i) % row_bl) * R + myrow) // local OWNED row -> local col
#define REMJ2GJ(j, pj)                                                         \
    ((pj)*col_bl + (j)) //((j)*C + (pj))    // col of proc (*,pj) -> global col
...
#define MSKGET(mask, n)                                                        \
    (mask[(n) / BITS(mask)] &                                                    \
     (1ULL << ((n) % BITS(mask)))) // get mask bit for vert at CSR index n
#define MSKSET(mask, n)                                                        \
    {                                                                            \
        mask[(n) / BITS(mask)] |= (1ULL << ((n) % BITS(mask)));                    \
    } // set mask bit for vert at CSR index n
...
#define LINKAGE
#endif
extern LINKAGE void setcuda(uint64_t ned, LOCINT *col, LOCINT *row,
                            LOCINT reach_v0);
extern LINKAGE void setcuda_2degree(LOCINT reach_v0, LOCINT reach_v1,
                                    LOCINT reach_v2);
...
extern LINKAGE LOCINT scan_col_csc_cuda(LOCINT *rbuf, LOCINT ld, int *rnum,
                                        int np, LOCINT *sbuf, int *snum,
                                        LOCINT *frt, LOCINT *frt_sigma,
                                        int level);

extern LINKAGE LOCINT scan_col_csc_cuda_mono(int ncol, int level);

extern LINKAGE LOCINT scan_frt_csc_cuda(const LOCINT *__restrict__ frt,
                                        int ncol, int depth, float *hSRbuf);

extern LINKAGE LOCINT scan_frt_csc_cuda_mono(int offset, int ncol, int depth);
extern LINKAGE LOCINT scan_frt_csc_cuda_mono_2degree(int offset, int ncol,
                                                     int depth, short branch,
                                                     LOCINT v2dg);

extern LINKAGE LOCINT write_delta_cuda(LOCINT ncol, float *hRFbuf,
                                       float *hSFbuf);

extern LINKAGE LOCINT append_rows_cuda(LOCINT *rbuf, LOCINT ld, int *rnum,
                                       int np, LOCINT *frt, LOCINT *frt_sigma,
                                       LOCINT nfrt, int level);

extern LINKAGE void update_bc_cuda(uint64_t v0, int ncol,
                                   const uint64_t nvisited);
extern LINKAGE void update_bc_cuda_2degree(uint64_t v0, uint64_t v1,
                                           uint64_t v2, int ncol,
                                           const uint64_t nvisited_v0,
                                           const uint64_t nvisited_v1,
                                           const uint64_t nvisited_v2);
...
extern LINKAGE void pred_reqs_cuda(LOCINT min, LOCINT max, LOCINT *sbuf,
                                   LOCINT ld, int *snum);
