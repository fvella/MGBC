...
void writeStats()
{
    ...
    */
    ...
    if (loc_count > 0)
        fprintf(
            fout,
            "N.\tID\tDegree\tLevel\tVisited\tTot_T\tTot_CUDA\tTot_MPI\tUpw_C\tUpw_M"
            "\tDep_C\tDep_M\tUpd_C\tUpd_M\tOverL\t\tNFRT\n");
    else
        fprintf(fout, "No vertex processed!");

    for (i = 0; i < loc_count; i++)
    {
        // Count, Vertex id, Degree, Level, Visited, Tot_Time, Tot_CUDA, Tot_MPI,
        // Upw_Cuda, Upw_Mpi, Dep_Cuda, Dep_Mpi, Upd_Cuda, Upd_Mpi, OverLap
        fprintf(fout, "%d\t%" PRIu64 "\t%d\t%d\t%" PRIu64 "\t%" PRIu64 "\t%" PRIu64
                "\t%" PRIu64 "\t%" PRIu64 "\t%" PRIu64 "\t%" PRIu64
                "\t%" PRIu64 "\t%" PRIu64 "\t%" PRIu64 "\t%" PRIu64 "\t",
                i, mystats[i].id, mystats[i].degree, mystats[i].lvl,
                mystats[i].visited, mystats[i].tot, mystats[i].compu,
                mystats[i].commu, mystats[i].upw[0], mystats[i].upw[1],
                mystats[i].dep[0], mystats[i].dep[1], mystats[i].upd[0],
                mystats[i].upd[1], mystats[i].over);
...
void upStats(uint64_t v0, int visited, int lvl, uint64_t msgSize,
             uint64_t upwc_time, uint64_t upwm_time, uint64_t depc_time,
             uint64_t depm_time, uint64_t updc_time, uint64_t updm_time,
             uint64_t ovrl_time, uint64_t compu_time, uint64_t commu_time,
             uint64_t total_time)
{
    if (VERT2PROC(v0) != myid)
        return;
...
static uint64_t select_root(LOCINT *col)
{
    ...
    do
    {
        ...
        if (pj == mycol)
            ld = col[GJ2LOCJ(c) + 1] - col[GJ2LOCJ(c)];
        else
            ld = 0;
...
static uint64_t select_root1(LOCINT *col)
{
    ...
    {
        lv = lrand48();
        MPI_Allreduce(&lv, &gv, 1, MPI_UNSIGNED_LONG_LONG, MPI_SUM,
                      MPI_COMM_CLUSTER);
        gv %= N;

        if (mycol == GJ2PJ(gv))
            ld = col[GJ2LOCJ(gv) + 1] - col[GJ2LOCJ(gv)];
        else
            ld = 0;
...
static void init_bc_1degree(uint64_t *edrem, uint64_t nedrem, uint64_t nverts,
                            LOCINT *reach)
{
...
static void check_recv_cols(LOCINT *rbuf, LOCINT ld, int *rnum, int n)
{
    ...
    for (i = 0; i < n; i++)
    {
        for (j = 0; j < rnum[i]; j++)
        {
            ...
            {
                fprintf(stderr, "[%d] V-Received vertex %" LOCPRI
                        " whose column does not belong to me!\n",
                        myid, rbuf[i * ld + j]);
                exit(EXIT_FAILURE);
            }
        }
...
static void check_recv_rows(LOCINT *rbuf, LOCINT ld, int *rnum, int n)
{
    ...
    for (i = 0; i < n; i++)
    {
        for (j = 0; j < rnum[i]; j++)
        {
            ...
            if (rbuf[i * ld + j] < mycol * row_bl ||
                rbuf[i * ld + j] >= (mycol * row_bl + row_bl))
            {
                fprintf(stderr, "[%d] H-Received vertex %" LOCPRI
                        " whose row does not belong to me!\n",
                        myid, rbuf[i * ld + j]);
                exit(EXIT_FAILURE);
            }
        }
...
static double bc_func(LOCINT *row, LOCINT *col, LOCINT *frt_all, LOCINT *frt,
                      int *hFnum, LOCINT *msk, int *lvl, LOCINT *deg,
                      LOCINT *sigma, LOCINT *frt_sig, float *delta,
                      LOCINT rem_ed, uint64_t v0, LOCINT *vRbuf, int *vRnum,
                      LOCINT *hSbuf, int *hSnum, LOCINT *hRbuf, int *hRnum,
                      float *hSFbuf, float *hRFbuf, LOCINT *reach,
                      MPI_Request *vrequest, MPI_Request *hrequest,
                      MPI_Status *status, uint64_t *total_time,
                      uint64_t *compu_time, uint64_t *commu_time, int dump)
{
    ...
    *commu_time = 0;
    ...
    while (1)
    {
        ...
        exchange_vert4x2(frt, frt_sig, nfrt, vRbuf, row_bl, vRnum, vrequest, status,
                         1);
        ncol = 0;
        ...
        for (i = 0; i < R; i++)
        {
            if (vRnum[i] > 0)
            {
                memcpy(frt_all + ncol, vRbuf + 2 * i * row_bl,
                       vRnum[i] * sizeof(*vRbuf));
                ncol += vRnum[i];
            }
        }
        ...
        // vRnum is an array of size R containing how many elements are in VRbuf for
        // each processor
        // NOTE: we cannot have duplicates in vRbuf since each processor can send only
        // nodes that it owns
        ...
        *commu_time += mpi_time;
        ...
        // Get neightbour vertices that have not been visited yet and put them into
        // hSbuf
        /*
        ...
        nfrt = scan_col_csc_cuda(vRbuf, row_bl, vRnum, R, hSbuf, hSnum, frt,
                                 frt_sig, level);
        ...
        *compu_time += cuda_time;
        ...
        exchange_horiz4x2(hSbuf, row_bl, hSnum, hRbuf, row_bl, hRnum, hrequest,
                          status, 1);
#ifdef _TIMINGS
        TIMER_STOP(1);
        mpi_time = TIMER_ELAPSED(1);
        ...
        *compu_time += cuda_time;
        ...
        MPI_Allreduce(&nfrt, &n, 1, MPI_UNSIGNED_LONG_LONG, MPI_SUM,
                      MPI_COMM_CLUSTER);
        ...
        *commu_time += mpi_time;
        ...
        if (n == 0)
            break; // Exit from the loop since we do not have new vertices to visit
        nvisited += n;
    ...
    if (ntask > 1)
    {
        ...
        // MPI_Allreduce(MPI_IN_PLACE, sigma, row_pp, LOCINT_MPI, MPI_SUM,
        // Row_comm);
        MPI_Allgather(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL, sigma, row_bl, LOCINT_MPI,
                      Row_comm);
        set_sigma(sigma);
        // sync cudaEventSynchronize( get_lvl_event );
        // MPI_Allreduce(MPI_IN_PLACE, lvl,   row_pp, MPI_INT,    MPI_SUM,
        // Row_comm);
    ...
    do
    {
        ...
        *compu_time += cuda_time;
        ...
        // local aggregated accumulation we have to use the frontier index since
        ...
        *compu_time += cuda_time;
        ...
        if (C > 1)
            // MPI_Allreduce(MPI_IN_PLACE, hSFbuf, row_pp, MPI_FLOAT, MPI_SUM,
            // Row_comm);
            MPI_Allgather(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL, hSFbuf, row_bl,
                          MPI_FLOAT, Row_comm);
    ...
    while (depth > 0);
    // All delta values have been calculated and stored into device buffer we need
    // to sum all them up
    ...
    if (mycol == 0)
    {
        //         if nvisited + rem_ed == N the graph is connected otherwise I have
        //         more connected components so I have to take into account only the
        //         reach of the vertices in the current connected compoent.
        //              if ((nvisited + rem_ed) != (N) ){
        //              pre_update_bc_cuda(lvl, reach, v0, &all);
        //              if (R>1)
        //                      MPI_Allreduce(MPI_IN_PLACE, &all, 1,
        //                      MPI_UNSIGNED_LONG_LONG, MPI_SUM, Col_comm);
        //                      printf("TOTAL %d ;  visited  %d ;  edge_rem %d :ONLY scc
        //                      %d\n",N, nvisited, rem_ed, );
        //              }else all = rem_ed;
    ...
    *total_time = TIMER_ELAPSED(0);
...
#ifdef _FINE_TIMINGS
    upStats(v0, nvisited, level, msgSize, upwc_time, upwm_time, depc_time,
            depm_time, updc_time, updm_time, overlap_time, *compu_time,
            *commu_time, *total_time);
#endif
    ...
    /*
        ...
        MPI_Reduce(&n, &ned, 1, MPI_UNSIGNED_LONG_LONG, MPI_SUM, 0,
       MPI_COMM_CLUSTER);
        ned >>= 1; // ??
    ...
    */
    ...
    if (dump) {
            ...
            dump_prd(prd, mycol*row_bl, mycol*row_bl+row_bl, hSbuf, row_bl, hSnum,
    msk);
    }
    */
    return teps;
...
static double bc_func_mono(LOCINT *row, LOCINT *col, LOCINT *frt_all,
                           LOCINT *frt, int *hFnum, LOCINT *msk, int *lvl,
                           LOCINT *deg, LOCINT *sigma, LOCINT *frt_sig,
                           float *delta, LOCINT rem_ed, uint64_t v0,
                           LOCINT *vRbuf, int *vRnum, LOCINT *hSbuf, int *hSnum,
                           LOCINT *hRbuf, int *hRnum, float *hSFbuf,
                           float *hRFbuf, LOCINT *reach, MPI_Request *vrequest,
                           MPI_Request *hrequest, MPI_Status *status,
                           uint64_t *total_time, int dump)
{
    ...
    */
    ...
    while (1)
    {
        ...
        // vRnum is an array of size R containing how many elements are in VRbuf for
        // each processor
        // NOTE: we cannot have duplicates in vRbuf since each processor can send
        // only nodes that it owns
...
#ifdef _FINE_TIMINGS
    upStats(v0, nvisited, level, msgSize, upwc_time, 0, depc_time, 0, updc_time,
            0, 0, 0, 0, *total_time);
#endif
...
static double bc_func_mono_2degree(LOCINT *row, LOCINT *col, LOCINT *frt_all,
                                   LOCINT *frt, int *hFnum, LOCINT *frt_all_v1,
                                   LOCINT *frt_v1, int *hFnum_v1, LOCINT *msk,
                                   int *lvl, LOCINT *deg, LOCINT *sigma,
                                   LOCINT *frt_sig, float *delta, LOCINT rem_ed,
                                   uint64_t v0, uint64_t v1, uint64_t v2,
                                   LOCINT *reach, uint64_t *total_time,
                                   int dump)
{
    ...
    *total_time = 0;
    ...
    if (test == 0 ){
    do {
        ncol_v1 = hFnum_v1[depth_v1+1] - hFnum_v1[depth_v1]; //number of nodes to
    process
            scan_frt_csc_cuda_mono_2degree(hFnum_v1[depth_v1], ncol_v1,
    depth_v1,0);
    depth_v1--;
    } while (depth_v1 > 0);

    do {
        ncol_v2 = hFnum[depth_v2+1] - hFnum[depth_v2]; //number of nodes to
    process
    ...
    pre_update_bc_cuda(reach, v0, &all_v0); // va fatta per 2-degree una sola
    // volta
    ...
    update_bc_cuda_2degree(v0, v1, v2, row_pp, nvisited_v1 + all_v0,
                           nvisited_v1 + all_v1, nvisited_v1 + all_v2);
...
#ifdef _FINE_TIMINGS
    upStats(v0, nvisited_v1 + 2, MAX(level_v1, level_v2) + 1, 0, upwc_time, 0,
            depc_time, 0, updc_time, 0, 0, 0, 0, *total_time);
#endif
    return 1;
}
        ...
        switch (c)
        {
            case 'H':
                ...
                if (heuristic >= 2 && ntask > 1)
                    prexit("2-degree Heuristic is allowed in single-gpu configuration "
                           "(-H): %s\n",
                           optarg);
                break;
            // heuristic selection
            ...
            case 'r':
                // CHECKRTYPE(0, 'r')
                if (0 == sscanf(optarg, "%" PRIu64, &startv))
...
#undef CHECKRTYPE
    ...
    if (heuristic >= 2 && ntask > 1)
    {
        prexit("\n\n2-degree Heuristic is allowed in single-gpu configuration "
               "(-H): %d\n",
               heuristic);
        MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
    }
    ...
    if (gmyid == 0)
    {
        ...
        //              fprintf(stdout, "Number of rows per block (N/(R*C)):
        //              %"LOCPRI"\n", row_bl);
        //              fprintf(stdout, "Number of columns per block (N/C):
        //              %"LOCPRI"\n", col_bl);
        //              fprintf(stdout, "Total rows per processor (N/R):
        //              %"LOCPRI"\n", row_pp);
        if (gread)
        {
    ...
    if (heuristic == 2 || heuristic == 3)
    {
        ...
        frt_all_v1 =
            (LOCINT *)CudaMallocHostSet(MAX(col_bl, row_pp) * sizeof(*frt), 0);
        hFnum_v1 =
            (int *)CudaMallocHostSet(MAX(row_pp, col_bl) * sizeof(*hFnum), 0);
    }
    ...
    if (!mono)
    {
        vRbuf = (LOCINT *)CudaMallocHostSet(
                    2 * col_bl * sizeof(*vRbuf), 0); // We need to double the size for sigma
        vRnum = (int *)CudaMallocHostSet(R * sizeof(*vRnum), 0);
        hSbuf = (LOCINT *)CudaMallocHostSet(
                    2 * row_pp * sizeof(*hSbuf), 0); // We need to double the size for sigma
        hSnum = (int *)CudaMallocHostSet(C * sizeof(*hSnum), 0);
        hRbuf = (LOCINT *)CudaMallocHostSet(
                    2 * row_pp * sizeof(*hRbuf), 0); // We need to double the size for sigma
        hRnum = (int *)CudaMallocHostSet(C * sizeof(*hRnum), 0);
        hSFbuf =
            (float *)CudaMallocHostSet(MAX(col_bl, row_pp) * sizeof(*hSFbuf), 0);
        hRFbuf =
            (float *)CudaMallocHostSet(MAX(col_bl, row_pp) * sizeof(*hRFbuf), 0);
        ...
        exchange_vert4x2(frt, frt_sigma, row_bl, vRbuf, row_bl, vRnum, vrequest,
                         status, 1);
        for (i = 0; i < C; i++)
            hSnum[i] = row_bl;
        exchange_horiz4x2(hSbuf, row_bl, hSnum, hRbuf, row_bl, hRnum, hrequest,
    ...
    uint64_t commu_all_time = 0, commu_time = 0, compu_all_time = 0,
             compu_time = 0;
    if (myid == 0)
        fprintf(stdout, "task %d: BC computation is running...\n", gmyid);
    if (heuristic == 2 || heuristic == 3)
    {
        ...
        while (nrounds < nbfs && (ui < N)) // for (ui = color; ui < nbfs; ui++)
        {
            ...
            else if (degree[ui] == 2)
            {
                ...
                if (prd[v1] != 0 || prd[v2] != 0)
                {
                    ...
                    bc_func_mono(row, col, frt_all, frt, hFnum, msk, lvl, degree, sigma,
                                 frt_sigma, delta, rem_ed, v0, vRbuf, vRnum, hSbuf, hSnum,
                                 hRbuf, hRnum, hSFbuf, hRFbuf, reach, vrequest, hrequest,
                                 status, &bc_time, 0);
                    all_time += bc_time;
                    prd[v0] = 1;
                    nrounds++;
                ...
                else
                {
                    ...
                    bc_func_mono_2degree(row, col, frt_all, frt, hFnum, frt_all_v1,
                                         frt_v1, hFnum_v1, msk, lvl, degree, sigma,
                                         frt_sigma, delta, rem_ed, v0, v1, v2, reach,
                                         &bc_time, 0);
    ...
    else     // NO H 2 o H 3
    {
        for (ui = color; ui < nbfs; ui += cntask)
        {
            ...
            if (ntask > 1)
                MPI_Allreduce(MPI_IN_PLACE, &skip, 1, LOCINT_MPI, MPI_SUM,
                              MPI_COMM_CLUSTER);
            ...
            if (ntask > 1)
                MPI_Allreduce(MPI_IN_PLACE, &reach_v0, 1, LOCINT_MPI, MPI_SUM,
                              MPI_COMM_CLUSTER);
            ...
            {
                // teps[ui]=
                bc_func(row, col, frt_all, frt, hFnum, msk, lvl, degree, sigma,
                        frt_sigma, delta, rem_ed, v0, vRbuf, vRnum, hSbuf, hSnum, hRbuf,
                        hRnum, hSFbuf, hRFbuf, reach, vrequest, hrequest, status,
                        &bc_time, &compu_time, &commu_time, 0);
            }
            else
            {
                // teps[ui]=
    ...
    if (mycol == 0)
    {
        ...
        if (color == 0)
        {
            MPI_Reduce(MPI_IN_PLACE, bc_val, row_pp, MPI_FLOAT, MPI_SUM, 0,
                       MPI_COMM_COL);
        }
        else
        {
    ...
    MPI_Reduce(&compu_all_time, &comp_avg_time, 1, MPI_UINT64_T, MPI_SUM, 0,
               MPI_COMM_CLUSTER);
    MPI_Reduce(&commu_all_time, &comm_avg_time, 1, MPI_UINT64_T, MPI_SUM, 0,
               MPI_COMM_CLUSTER);
    ...
    if (myid == 0)
    {
        ...
        fprintf(stdout, "ClusterId\tSkip\tRounds\tExecTime\tRoundsTime\tCUDATime("
                "avg)\tMPITime(avg)\t\tMax\tMin\tMean\t1-dReduTime\n");
        fprintf(stdout, "%d\t%d\t%d\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n",
                color, skipped, nrounds, (all_time + bcred_time) / 1.0E+6,
                all_time / 1.0E+6, comp_avg_time / 1.0E+6, comm_avg_time / 1.0E+6,
                max_time / 1.0E+6, min_time / 1.0E+6, all_time / 1.0E+6 / nrounds,
        ...
        fprintf(
            stdout,
            "task %d BC rounds Computation CUDA avg time: %lf sec over %d procs\n",
            gmyid, comp_avg_time / 1.0E+6, ntask);
        fprintf(
            stdout,
            "task %d BC rounds Communication MPI avg time: %lf sec over %d procs\n",
        ...
        if (heuristic == 2 || heuristic == 3)
        {
            fprintf(stdout, "task %d 2-Degree reduction: %lf sec.(2-degree nodes "
                    "computed by 2-degree fun %d/%d)\n",
