...
#undef CHECKRTYPE
    ...
    LOCINT two_dg_c = 0;
    LOCINT two_dg_n = 0;
    uint64_t all_time = 0, all_time2 = 0, bc_time = 0, triple_time = 0,
             triple_time2 = 0, min_time = UINT_MAX, max_time = 0;
    ...
    if (heuristic == 2 || heuristic == 3)
    {
        ...
        nbfs = dgT; // devo solamente calcolare il tempo di tutti di degree-2 con la
        // nostra tecnica
        while (nrounds < nbfs && (ui < N)) // for (ui = color; ui < nbfs; ui++)
        {
            ...
            bc_func_mono_2degree(row, col, frt_all, frt, hFnum, frt_all_v1, frt_v1,
                                 hFnum_v1, msk, lvl, degree, sigma, frt_sigma, delta,
                                 rem_ed, v0, v1, v2, reach, &bc_time, max_depth);
            triple_time = bc_time;
            all_time += bc_time;
            ...
            bc_func_mono(row, col, frt_all, frt, hFnum, msk, lvl, degree, sigma,
                         ...
                         status, &bc_time, max_depth);
            all_time2 += bc_time;
            triple_time2 += bc_time;

            setcuda(ned, col, row, reach_v1);
            bc_func_mono(row, col, frt_all, frt, hFnum, msk, lvl, degree, sigma,
                         frt_sigma, delta, rem_ed, v1, vRbuf, vRnum, hSbuf, hSnum,
                         hRbuf, hRnum, hSFbuf, hRFbuf, reach, vrequest, hrequest,
                         status, &bc_time, max_depth);
            all_time2 += bc_time;
            triple_time2 += bc_time;
            ...
            bc_func_mono(row, col, frt_all, frt, hFnum, msk, lvl, degree, sigma,
                         frt_sigma, delta, rem_ed, v2, vRbuf, vRnum, hSbuf, hSnum,
                         hRbuf, hRnum, hSFbuf, hRFbuf, reach, vrequest, hrequest,
                         status, &bc_time, 0);
            all_time2 += bc_time;
            triple_time2 += bc_time;
            // FINE TRIPLA
            nrounds++;
            printf("[Benchmark2d] Triple[%u] vertices %lu %lu %lu - TIME  DMF %lf -  "
                   "MGBC %lf - secs\n",
    ...
    if (myid == 0)
    {
        ...
        {
            fprintf(stdout,
                    "[Benchmark2d] Degree-1 Reduction Preprocessing %lf sec\n",
                    degree_reduction_time / 1.0E+6);
        }
        if (heuristic == 2 || heuristic == 3)
        {
            fprintf(stdout,
                    "[Benchmark2d] Degree-2 Sorting Preprocessing  %lf secs\n",
            ...
            fprintf(stdout, "[Benchmark2d] DMF IMP %lf percent\n",
                    100 - (((float)(all_time / 1.0E+6 / nrounds) * 100) /
                           (float)(all_time2 / 1.0E+6 / nrounds)));
            fprintf(stdout, "[Benchmark2d] DMF EFFICIENCY %lf percent\n",
                    100 * (100 - (((float)(all_time / 1.0E+6 / nrounds) * 100) /
                                  (float)(all_time2 / 1.0E+6 / nrounds))) /
                    32.18);
        }
        fprintf(stdout, "\n");
