...
#ifdef GRAPH_GENERATOR_MPI
void make_graph(int log_numverts, int64_t desired_nedges, uint64_t userseed1, uint64_t userseed2, const double initiator[4], int64_t* nedges_ptr, int64_t** result_ptr, MPI_Comm ICOMM) {
  int64_t N, M;
  int rank, size;
  ...
  rand_sort_mpi(ICOMM, &state, N, &perm_local_size, &local_vertex_perm);
  ...
  *result_ptr = result;
  ...
  start = MPI_Wtime();
  apply_permutation_mpi(ICOMM, perm_local_size, local_vertex_perm, N, nedges, result);
  ...
  int64_t* new_result;
  int64_t nedges_out;
  scramble_edges_mpi(ICOMM, userseed1, userseed2, nedges, result, &nedges_out, &new_result);
